...
public class RunLoop implements Closeable {
  ...
  private static final String KCF_RUN_LOOP_DEFAULT_MODE =
  ...
  public synchronized static boolean executingOnRunLoop() {
    return ACTIVE_RUNLOOPS
        .containsKey(Long.valueOf(Thread.currentThread().getId()));
  }
  ...
  public synchronized static boolean executingOnDefaultRunLoop() {
    RunLoop rl =
        ACTIVE_RUNLOOPS.get(Long.valueOf(Thread.currentThread().getId()));
    if (rl == null) {
      return false;
    }
   ...
   * </p>
   *
   * @param setDefault whether to set the returned runloop
      ...
      synchronized (RunLoop.class) {
        if (getDefault().isPresent()) {
          throw new IllegalStateException(
              "Another run loop is already the default");
        }
        setDefault(this);
      }
    }
  ...
  public <T> T enterInternal(Callable<T> task, CountDownLatch initLatch)
      throws Exception {
    if (running) {
    ...
    try {
      ...
      try {
        Future<T> future =
            executorService.submit(new WrappedCallable<>(task, initLatch));
  ...
  private CFStringRef resolveNativeProperty(String prop) {
    return new CFStringRef(
        CoreMidiServiceFactory.getNativeLibrary().getGlobalVariableAddress(prop)
            .getPointer(0));
  }

  private void runloop(Future<?> future, CountDownLatch initLatch)
    ...
    try {
      ...
      while (!future.isDone()) {
        ...
        if (elapsed > 1000) {
          LOG.trace(
              "Called CFRunLoopRunInMode {} times: {} finished, {} stopped, {} timed out, {} other",
  ...
  private ExecutorService executorService() {
    return Executors.newFixedThreadPool(1, r -> {
      ...
      t.setName(String
          .format("runloop-task-%d", TASK_ID_GENERATOR.incrementAndGet()));
      t.setDaemon(false);
