...
public class AbletonPush2 implements Closeable {
  ...
  private static final int BYTES_PER_ROW =
      COLS * BYTES_PER_PIXEL + PADDING_PER_ROW;
  ...
  private static final WritablePixelFormat<ByteBuffer> BGRA_FORMAT =
      WritablePixelFormat.getByteBgraPreInstance();

  private static final byte[] FRAME_HEADER =
      new byte[] { (byte) 0xff, (byte) 0xcc, (byte) 0xaa, (byte) 0x88, 0x00,
  ...
  public void sendFrameHeader() throws IOException {
    try {
      ...
      usb.bulkTransfer(pushDevice, PUSH2_BULK_EP_OUT, frameHeader,
          FRAME_HEADER.length, transferred, TRANSFER_TIMEOUT);
  ...
  public void sendActiveFramebuffer() throws IOException {
    ...
    for (int i = 0; i < BUFFER_SIZE; i += TRANSFER_SIZE) {
      try {
        ...
        usb.bulkTransfer(pushDevice, PUSH2_BULK_EP_OUT, active.share(i),
            TRANSFER_SIZE, transferred, TRANSFER_TIMEOUT);
  ...
  private static Pointer openPush(Usb usb, Pointer usbContext)
      throws IOException {
    ...
    try {
      ...
      UsbDeviceDescriptor pushDescriptor = Arrays.stream(udds.descriptors)
          ...
          .filter(udd -> udd.idProduct == PUSH2_PRODUCT_ID).findFirst()
          .orElseThrow(
              () -> new IOException("Ableton Push 2 device not found"));
  ...
  private static void claimInterface(Usb usb, Pointer pushDevice)
      throws IOException {
    try {
  ...
  public static int colorToArgb(int red, int green, int blue) {
    return ((0xFF & 0xFF) << 24) | ((red & 0xFF) << 16) | ((green & 0xFF) << 8)
        | ((blue & 0xFF) << 0);
  }
  ...
  public static class JavaFX extends Application {
    ...
    @Override public void start(Stage stage) throws Exception {
      ...
      webview.getEngine()
          .load("https://player.vimeo.com/video/48070605?autoplay=1");
