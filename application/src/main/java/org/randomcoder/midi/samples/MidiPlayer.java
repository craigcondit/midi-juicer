...
public class MidiPlayer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    var deviceInfos = Arrays
        .stream(MidiSystem.getMidiDeviceInfo())
        .filter(
            di -> di.getDescription().toLowerCase(Locale.US).contains("usb"))
    ...
    if (args.length < 2) {
      System.err.printf("Usage: %s <midi-file> <volume>%n",
    ...
    try (MidiDevice device = devices.get(0);
      ...
      try (Sequencer seq = MidiSystem.getSequencer(false);
          Transmitter tx = seq.getTransmitter()) {
        seq.open();
