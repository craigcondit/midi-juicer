...
public class CFStringRefTest {
  ...
  @Test public void emptyConstructorReferencesNullPointer() {
    CFStringRef ref = new CFStringRef();
    assertEquals(0L, Pointer.nativeValue(ref.getValue()));
  }

  @Test public void pointerConstructorRetainsValue() {
    Memory m = new Memory(8);
    CFStringRef ref = new CFStringRef(m);
    assertEquals(m, ref.getPointer());
  }

  @Test public void createNativeCallsCFStringCreateWithCharacters() {
  ...
  @Test public void toStringWithZeroLengthReturnsEmptyString() {
  ...
  @Test public void toStringHandlesEncodingProperly() {
    ...
    doAnswer(i -> {
      assertSame("first argument must be the object under test", obj,
          i.getArgument(0));
      ...
      return null;
    }).when(cf).CFStringGetCharacters(any(CFStringRef.class),
        any(CFRange.ByValue.class), any(Memory.class));
