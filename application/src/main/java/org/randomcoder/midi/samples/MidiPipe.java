...
public class MidiPipe {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try (RunLoop rl = RunLoop.spawn(true)) {
      ...
      for (MidiDevice.Info info : infos) {
        LOG.info("Found device: {}, vendor={}, description={}", info.getName(),
      ...
      if (input.isPresent() && output.isPresent()) {
        try (MidiDevice inDev = input.get();
            Transmitter transmitter = inDev.getTransmitter()) {

          LOG.debug("Opened transmitter for device: {}", inDev);
          try (MidiDevice outDev = output.get();
            ...
            MidiFilter filter = (m, t) -> {
              ...
              if (sm.getStatus() == ShortMessage.NOTE_ON
                  || sm.getStatus() == ShortMessage.NOTE_OFF) {
  ...
  private static Optional<MidiDevice> getInput(MidiDevice.Info[] infos) {
    ...
    return deviceInfos.stream().map(di -> {
      try {
