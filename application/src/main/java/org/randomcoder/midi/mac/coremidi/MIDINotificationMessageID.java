...
public enum MIDINotificationMessageID {
  kMIDIMsgUnknown(-1,
      (p, o, l) -> new MIDINotification.Unknown()), kMIDIMsgSetupChanged(1,
      (p, o, l) -> new MIDINotification.SetupChanged()), kMIDIMsgObjectAdded(2,
      (p, o, l) -> {
        if (l < 16) {
          // invalid size, just ignore it
          return new MIDINotification.Unknown();
        }
        return new MIDINotification.ObjectAdded(p.getInt(o),
            MIDIObjectType.byValue(p.getInt(o + 4)), p.getInt(o + 8),
    ...
    return new MIDINotification.ObjectRemoved(p.getInt(o),
        MIDIObjectType.byValue(p.getInt(o + 4)), p.getInt(o + 8),
        MIDIObjectType.byValue(p.getInt(o + 12)));
    ...
    return new MIDINotification.PropertyChanged(p.getInt(o),
        MIDIObjectType.byValue(p.getInt(o + 4)), cfstr.toString());
  }), kMIDIMsgThruConnectionsChanged(5,
      (p, o, l) -> new MIDINotification.ThruConnectionsChanged()), kMIDIMsgSerialPortOwnerChanged(
      6,
      (p, o, l) -> new MIDINotification.SerialPortOwnerChanged()), kMIDIMsgIOError(
      7, (p, o, l) -> {
    if (l < 8) {
      // invalid size, just ignore it
      return new MIDINotification.Unknown();
  ...
  private MIDINotificationMessageID(int value,
      MIDINotificationCreator creator) {
    this.value = value;
  ...
  public static MIDINotificationMessageID byValue(final int value) {
    return Arrays.stream(MIDINotificationMessageID.values())
        .filter(e -> e.value == value).findFirst().orElse(kMIDIMsgUnknown);
  }
}
