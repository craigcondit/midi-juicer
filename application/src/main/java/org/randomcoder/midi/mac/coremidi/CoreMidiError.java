...
public enum CoreMidiError {
  ...
  kNoError(0, "Success"), kUnknownError(-65536,
      "Unknown error"), kMIDIInvalidClient(-10830,
      ...
      -10833,
      "Attempt to close a non-existant connection."), kMIDIUnknownEndpoint(
      -10834, "An invalid MIDIEndpointRef was passed."), kMIDIUnknownProperty(
      -10835,
      ...
      -10838, "Communication with MIDIServer failed."), kMIDIServerStartErr(
      -10839, "Unable to start MIDIServer."), kMIDISetupFormatErr(-10840,
      "Unable to read the saved state."), kMIDIWrongThread(-10841,
      ...
      -10842, "The requested object does not exist."), kMIDIIDNotUnique(-10843,
      "Attempt to set a non-unique kMIDIPropertyUniqueID on an object."), kMIDINotPermitted(
      -10844, "Operation not permitted.");
  ...
  public static CoreMidiError byErrorCode(final int intValue) {
    return Arrays.stream(CoreMidiError.values())
        .filter(e -> e.errorCode == intValue).findFirst().orElse(kUnknownError);
  }
